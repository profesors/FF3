...
\renewcommand{\spanishabstractname}{Prólogo}
...
\begin{document}
    ...
    \begin{equation}    \label{eq:calcula_de_a}
    ...
    con los valores obtenidos en las ecuaciones \ref{eq:radio_nucleo_atomico} y \ref{eq:calcula_de_a}. En caso de proceder
    de esta manera obtenemos el resultado \num{1.2183d-6}. No obstante, vamos a desarrollar la integral. Tenga el lector
    el valor suficiente como para acompañarnos bien pertechado con una buena dosis de café si fuera necesario. Recuerde
    el lector que el símbolo matemático $\therefore$ significa por lo tanto.
    ...
    Si sustituimos en este resultado por los valores de las ecuaciones \ref{eq:radio_nucleo_atomico} y \ref{eq:calcula_de_a}
    volvemos a obtener el resultado \num{1.2183d-6}, como obtuvimos con la calculadora.
    %$$\int_{0}{R} r^2 \cdot e^{-\frac{2}{a}r} dr$$
    ...
    ecuación \label{eq:calcula_de_a}, por otra parte los valores $r_0$ y $A$ están indicados en el enunciado.
    ...
    El resultado lo podemos ver en el gráfico \ref{gr:probabilidad_radio_electron}

    \begin{figure}[ht]
      \centering
      \caption{Probabilidad de encontrar el electrón en una posición concreta} \label{gr:probabilidad_radio_electron}
        \fontsize{8}{12}\selectfont
        \resizebox{1\textwidth}{!}{\input{grafProbabilidadRadioElectron}}
    \end{figure}
    \FloatBarrier   % Para que el gráfico no se vaya a la siguiente sección
